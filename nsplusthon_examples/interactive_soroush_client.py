...
from nsplusthon.errors import SessionPasswordNeededError
from nsplusthon.network import ConnectionTcpAbridged
from nsplusthon.utils import get_display_name
...
class InteractiveSoroushClient(SoroushClient):
    """Full featured SoroushPlus client, meant to be used on an interactive
       session to see what NSplusthon is capable off -

       This client allows the user to perform some basic interaction with
       SoroushPlus through NSplusthon, such as listing dialogs (open chats),
       talking to people, downloading media, and receiving updates.
    """

    def __init__(self, session_user_id, api_id=None, api_hash=None,
                 proxy=None):
        """
        Initializes the InteractiveSoroushClient.
        :param session_user_id: Name of the *.session file.
        :param api_id: Optional. NSplusthon default is used if omitted.
        :param api_hash: Optional. NSplusthon default is used if omitted.
        ...
        # The first step is to initialize the SoroushClient, as we are
        # subclassing it, we need to call super().__init__(). On a more
        # normal case you would want 'client = SoroushClient(...)'
        kwargs = {}
    ...
    async def run(self):
        """Main loop of the SoroushClient, will wait for user action"""
    ...
    def download_progress_callback(downloaded_bytes, total_bytes):
        InteractiveSoroushClient.print_progress(
...
async def main():
    SESSION = os.environ.get('SPLUS_SESSION', os.environ.get('TG_SESSION', 'interactive'))
    client = InteractiveSoroushClient(SESSION)
    await client.init()
    await client.run()
