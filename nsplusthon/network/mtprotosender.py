...
class MTProtoSender:
        ...
        for attempt in retry_range(retries, force_retry=False):
            try:
                ...
                if isinstance(e, InvalidBufferError) and e.code == 404:
                    self._log.info('Server does not know about the current auth key; clearing and regenerating')
        ...
        while self._user_connected and not self._reconnecting:
            ...
            try:
                ...
                if isinstance(e, InvalidBufferError) and e.code == 404:
                    self._log.info('Server does not know about the current auth key; clearing and regenerating')
                    self.auth_key.key = None
                    self._start_reconnect(AuthKeyNotFound())
                else:
                    self._log.warning('Invalid buffer %s', e)
                    self._start_reconnect(e)
                return
    ...
    async def _handle_rpc_result(self, message):
        ...
        if rpc_result.error:
            ...
            self._log.warning('RPC error for msg %d: %s (request=%s)',
                              rpc_result.req_msg_id, error,
                              state.request.__class__.__name__ if state.request else '?')
