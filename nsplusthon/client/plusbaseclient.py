...
from ..network import MTProtoSender, Connection, ConnectionWebSocket, TcpMTProxy
...
DEFAULT_DC_ID = 3
DEFAULT_IPV4_IP = 'im-server.splus.ir'
DEFAULT_IPV6_IP = '2001:067c:04e8:f004:0000:0000:0000:000a'
DEFAULT_PORT = 443
...
class SoroushPlusBaseClient(abc.ABC):
    ...
    Arguments
        session (`str` | `nsplusthon.sessions.abstract.Session`, `None`):
        ...
        api_id (`int` | `str`):
            Application identifier sent in ``initConnection``. Soroush Plus
            does **not** require registering at my.telegram.org — the
            library default is used if you omit this.

        api_hash (`str`):
            Application hash sent in ``initConnection``. Optional; the
            library default is used if you omit this.

        connection (`nsplusthon.network.connection.common.Connection`, optional):
            The connection instance to be used when creating a new connection
            to the servers. It **must** be a type.

            Defaults to `nsplusthon.network.connection.tcpfull.ConnectionTcpFull`.
        ...
        raise_last_call_error (`bool`, optional):
            When API calls fail in a way that causes NSplusthon to retry
        ...
        app_version (`str`, optional):
            ...
            Defaults to `nsplusthon.version.__version__`.
    ...
    def __init__(
            ...
            *,
            connection: 'typing.Type[Connection]' = ConnectionWebSocket,
            use_ipv6: bool = False,
            ...
            app_version: str = '3.9.2 A',
            lang_code: str = 'fa',
            system_lang_code: str = 'fa',
        ...
        if not api_id or not api_hash:
            raise ValueError(
                ...
                "Refer to nsplusthon.rtfd.io for more information.")
    ...
    @property
    def loop(self: 'SoroushClient') -> asyncio.AbstractEventLoop:
        """
    ...
    @property
    def disconnected(self: 'SoroushClient') -> asyncio.Future:
        """
    ...
    async def connect(self: 'SoroushClient') -> None:
        ...
        .. note::
            ...
            request, like `client.get_me() <nsplusthon.client.users.UserMethods.get_me>`,
        ...
        if self.session is None:
            raise ValueError('SoroushClient instance cannot be reused after logging out')
        ...
        # get_me) returns 500 INTERNAL_SERVER_ERROR which can poison
        # the WebSocket connection before the login flow begins.
        # The start() flow handles login and get_me() after auth.
    ...
    async def _save_states_and_entities(self: 'SoroushClient'):
    ...
    async def _disconnect_coro(self: 'SoroushClient'):
        if self.session is None:
    ...
    async def _disconnect(self: 'SoroushClient'):
        """
    ...
    async def _switch_dc(self: 'SoroushClient', new_dc):
        """
    ...
    async def _auth_key_callback(self: 'SoroushClient', auth_key):
        """
    ...
    class _SoroushDcOption:
        """Simple DC option for Soroush server mapping."""
        def __init__(self, dc_id, ip_address, port):
            self.id = dc_id
            self.ip_address = ip_address
            self.port = port
            self.ipv6 = ':' in ip_address
            self.cdn = False
            self.media_only = False
            self.tcpo_only = False
            self.static = False
            self.this_port_only = False
            self.secret = None

    async def _get_dc(self: 'SoroushClient', dc_id, cdn=False):
        """
        Gets the Data Center (DC) associated to 'dc_id'.
        For Soroush, all DCs are routed through im-server.splus.ir.
        """
        ...
        if not cdn:
            ...
            if ip is not None:
                return self._SoroushDcOption(dc_id, ip, 443)

        # Fall back to config lookup (for CDN or unknown DCs)
        cls = self.__class__
        if not cls._config:
    ...
    async def _create_exported_sender(self: 'SoroushClient', dc_id):
        ...
        """
        # Thanks badoualy/kotlogram on /telegram/api/DefaultSoroushClient.kt
    ...
    async def _borrow_exported_sender(self: 'SoroushClient', dc_id):
        """
    ...
    async def _return_exported_sender(self: 'SoroushClient', sender):
        """
    ...
    async def _clean_exported_senders(self: 'SoroushClient'):
        """
    ...
    async def _get_cdn_client(self: 'SoroushClient', cdn_redirect):
    ...
    def __call__(self: 'SoroushClient', request, ordered=False):
        ...
        Args:
            ...
            flood_sleep_threshold (`int` | `None`, optional):
                ...
                `client.flood_sleep_threshold <nsplusthon.client.plusbaseclient.SoroushPlusBaseClient.flood_sleep_threshold>`
    ...
    @abc.abstractmethod
    def _update_loop(self: 'SoroushClient'):
        raise NotImplementedError
