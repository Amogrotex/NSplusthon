...
class UpdateMethods:
    ...
    async def set_receive_updates(self: 'SoroushClient', receive_updates):
        """
    ...
    def on(self: 'SoroushClient', event: EventBuilder):
        """
    ...
    def add_event_handler(
        ...
        Arguments
            callback (`callable`):
                ...
                Note that if you have used `nsplusthon.events.register` in
            ...
            event (`_EventBuilder` | `type`, optional):
                ...
                If left unspecified, `nsplusthon.events.raw.Raw` (the
    ...
    def list_event_handlers(self: 'SoroushClient')\
    ...
    async def catch_up(self: 'SoroushClient'):
        """
    ...
    async def _update_loop(self: 'SoroushClient'):
        ...
        try:
                ...
                if get_diff:
                    ...
                    try:
                        ...
                        self._log[__name__].warning(
                            ...
                            f"in NSplusthon v{__version__} in that it could be handled better, but it's unlikely): %s",
                            e
                        )
                        self._message_box.end_difference()
                        continue
            ...
            self._log[__name__].exception(f'Fatal error handling updates (this is a bug in NSplusthon v{__version__}, please report it)')
            self._updates_error = e
            await self.disconnect()
    ...
    async def _dispatch_event(self: 'SoroushClient', event):
        """
...
class EventBuilderDict:
    ...
    """
    def __init__(self, client: 'SoroushClient', update, others):
        self.client = client
        self.update = update
        self.others = others
